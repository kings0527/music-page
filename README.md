...
- 顺序、列表循环和随机播放；播放模式会保存在浏览器中。
- 按当前音频时间同步滚动歌词。
- AAC 流媒体优先，MP3 兼容回退，WAV 母版保留下载。
- “缓存本曲”把完整流媒体文件校验后保存到 Cache Storage；再次点击可重新下载。
...
当前 WAV 是 48 kHz 双声道 PCM，大小约 45.8 MB，需要持续约 192 KB/s。
网页改用 160 kbps AAC（约 4.9 MB），并提供 192 kbps MP3（约 5.7 MB）
作为回退。GitHub Pages 和离线缓存都支持标准 Range 请求，因此无需把 WAV
手工拆成许多小文件；压缩流媒体对首播缓冲的改善更直接。
